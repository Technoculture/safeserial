...
def mock_bridge(monkeypatch):
    ...
    # Patch the internal _core.SerialPort class used by DataBridge
    # We need to verify how DataBridge instantiates it. 
    # It does: self._serial = _core.SerialPort()
    # So we patch _core.SerialPort to return our mock instance.
    
    monkeypatch.setattr(data_bridge._core, "SerialPort", lambda: mock_serial)
    
    bridge = data_bridge.DataBridge()
    bridge.mock_serial = mock_serial # Attach for inspection
    return bridge

def test_databridge_open_send(mock_bridge):
    assert mock_bridge.open("/dev/test")
    assert mock_bridge.is_open()
    
    mock_bridge.send("Test Payload")
    
    # Verify write was called
    assert len(mock_bridge.mock_serial._write_log) == 1
    sent_pkt = mock_bridge.mock_serial._write_log[0]
    ...
    mock_bridge.close()
    assert not mock_bridge.is_open()
...
def test_databridge_receive(mock_bridge):
    ...
    mock_bridge.open("/dev/test")
    mock_bridge.on('data', on_data)
    ...
    mock_bridge.mock_serial.queue_read(pkt)
    ...
    # Verify ACK was sent
    assert len(mock_bridge.mock_serial._write_log) == 1
    ack_frame, _ = _core.Packet.deserialize(mock_bridge.mock_serial._write_log[0])
