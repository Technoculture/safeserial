...
import { ResilientDataBridge } from "../lib/index";
...
describe("Native Wrapper Behavior", () => {
  it("should remain disconnected on invalid port when reconnect disabled", async () => {
    const bridge = await ResilientDataBridge.open("/dev/doesnotexist", {
      reconnect: false,
    });
  ...
  it("should throw on send when reconnect disabled and not connected", async () => {
    const bridge = await ResilientDataBridge.open("/dev/doesnotexist", {
      reconnect: false,
    });

    await expect(bridge.send("payload")).rejects.toThrow();
  });
});
