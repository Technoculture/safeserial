...
def parse_test_report(report_path: str) -> dict:
    ...
    for test_id, sender_log, receiver_log in log_matches:
        config = test_configs.get(
            test_id, {"drop_rate": 0, "corrupt_rate": 0, "expected_items": 20}
        )
...
def generate_fault_tolerance_chart(results: dict, output: str = "test_timeline.png"):
    ...
    bars1 = ax1.bar(
        x - width, sends, width, label="Fragments Sent", color="#3498db", alpha=0.8
    )
    bars2 = ax1.bar(
        x, retries, width, label="Retries (Recovery)", color="#e74c3c", alpha=0.8
    )
    bars3 = ax1.bar(
        x + width,
        deliveries,
    ...
    ax1.set_title(
        "Protocol Recovery Under Increasing Chaos", fontsize=12, fontweight="bold"
    )
    ax1.set_xticks(x)
    ...
    for test in results["tests"]:
        ...
        retry_rates.append(
            100 * test["retries"] / test["sends"] if test["sends"] > 0 else 0
        )
...
def generate_summary_stats(results: dict):
    ...
    print(
        f"Retry rate: {total_retries / total_sends * 100:.1f}%"
        if total_sends > 0
        else "N/A"
    )
    print(
        f"Delivery success: {'100% ✓' if total_receives == expected_total else 'FAILED'}"
    )
