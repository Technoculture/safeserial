...
def run_test_cycle(sender_lang, receiver_lang, drop_rate, corrupt_rate, item_count):
    print(f"--- Starting Test Cycle: Sender={sender_lang} -> Receiver={receiver_lang} "
    ...
    try:
        ...
        try: sender_out, _ = sender_proc.communicate(timeout=1)
        except: pass
...
def main():
    ...
    parser.add_argument("--sender", choices=["cpp", "node", "python"], default="cpp", help="Sender implementation")
    parser.add_argument("--receiver", choices=["cpp", "node", "python"], default="cpp", help="Receiver implementation")
    parser.add_argument("--target", help="Legacy: set both sender and receiver to this value", choices=["cpp", "node", "python"])
    ...
    try:
        for scenario in test_scenarios:
            passed, logs = run_test_cycle(sender, receiver, scenario["drop"], scenario["corrupt"], scenario["items"])
            results.append({
                "drop": scenario["drop"],
